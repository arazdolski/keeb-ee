...
const Footer: React.FC<FooterProps> = ({ logoUrl, innerLogoUrl, imageUrl }) => {
  return (
    <footer className='flex justify-around items-center space-x-36 p-6'>
      ...
      <div className='fixed bottom-6 right-6 mb-4 mr-4 z-40'>
        <Image
          width={1200}
          height={800}
          src={logoUrl}
          alt='logo'
          className='h-20 w-20 md:h-40 md:w-40 slow-spin'
        />
        <a
        ...
        >
          <Image
            ...
            height={800}
            alt='inner logo'
            className='absolute h-16 w-16 md:h-36 md:w-36 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2'
          />
        </a>
      </div>
    </footer>
